...
import { SKINS, SKIN_UNLOCK_SCORE } from "@/components/cell/logic/skinData";

export default function SkinPicker({ score = 0, currentSkin = null, onSelectSkin, inline = false }) {
  ...
  // Shared skin grid used by both inline and dropdown modes
  const SkinGrid = (
    <>
      {nextLock && (
        <p className="border-b border-slate-800 px-4 py-1.5 text-xs text-slate-400">
          Next unlock:{" "}
          <span className="font-semibold text-slate-200">{nextLock.name}</span> at{" "}
          ...
          {SKINS.map((skin) => {
            ...
            return (
              <button
                ...
                title={isLocked ? `Unlocks at ${skin.unlockScore} pts` : skin.name}
              ...
              >
                <div className="absolute inset-0" style={{ background: skin.gradient }} />
  ...
  // Inline mode: render grid directly (used inside PlayerCard dropdown)
  if (inline) {
    if (!anyUnlocked) {
      return (
        <p className="px-3 py-2 text-xs text-slate-500">
          Reach {SKIN_UNLOCK_SCORE} pts to unlock skins
        </p>
      );
    }
    return <div>{SkinGrid}</div>;
  }
  ...
  return (
    <div className="relative shrink-0">
      <button
        ...
        title={anyUnlocked ? "Choose a skin" : `Reach ${SKIN_UNLOCK_SCORE} score to unlock skins`}
      ...
      >
        ...
        <span>Skins</span>
        {!anyUnlocked && (
          <span className="text-xs text-slate-500">{score}/{SKIN_UNLOCK_SCORE}</span>
        )}
      </button>
      ...
      {open && anyUnlocked && (
        <div className="absolute left-0 top-full z-50 mt-1 w-[348px] rounded-xl border border-slate-700 bg-slate-900 shadow-2xl">
          <div className="flex items-center justify-between border-b border-slate-700 px-4 py-2">
            ...
            <button onClick={() => setOpen(false)} className="text-slate-400 transition hover:text-white" aria-label="Close skins">
              ✕
            </button>
          </div>
          {SkinGrid}
